...
API_KEY = 'fd58e9a82c0e0ea7230785163f68f7ef'
API_SECRET = 'fae5c99afb5f7a98'
email = None
password = None
AUTH = False
debug = False
...
class Auth():
    ...
    def loginLink(self, permission):
        """Generates a link that the user should be sent to"""
        myAuth = Auth()
        sig_str = API_SECRET + 'api_key' + API_KEY + 'perms' + permission
        signature_hash = hashlib.md5(sig_str).hexdigest()
        perms = permission
        link = "http://flickr.com/services/auth/?api_key=%s&perms=%s&api_sig=%s" % (API_KEY, perms, signature_hash)
        return link
